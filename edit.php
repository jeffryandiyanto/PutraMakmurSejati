...
<html>
    <head>
		<title>Putra Makmur Sejati | Edit Post</title>
			...
			<div class="row d-flex justify-content-center">
				<div class="col-lg-8 col-md-8">
					...
					if (isset($_GET['product_id'])){
						$id = $_GET['product_id'];
						$show = mysqli_query($connection, "SELECT * FROM `product` WHERE `productID` = '$id'");
						while($product = mysqli_fetch_array($show)) {
					...
					if (isset($_POST['post'])) {
						...
						"UPDATE `product` SET `namaProduct`='$namaProduct',
						`date`='$dateProduct',
						`gambarProduct` ='$newfilename',
						`detail`='$detail',
						`version`='$version',
						`price`='$price',
						`genreProduct`='$genre'
						WHERE `productID` = '$id'");
						if ($sql) echo '<meta http-equiv="refresh" content="0;url=home.php">';
					}
					?>
				</div>
			</div>
